// Jigyasa News - Production-Ready Script
...
function renderResult(result) {
  ...
  return `
    <div class="result-card verdict-${result.verdict}">
      <h3>${verdictTexts[result.verdict]}</h3>
      <span class="verdict-label">${verdictLabels[result.verdict]}</span>
      <p>${result.explanation}</p>
      <div class="confidence-bar">
        <div class="confidence-fill" style="width: ${result.confidence}%"></div>
      </div>
      <p style="font-size: var(--text-sm); color: var(--color-text-muted);">Confidence: ${result.confidence}%</p>
      <div class="sources">
...
function showLoading() {
  ...
  resultsContent.innerHTML = `
    <div class="result-card loading">
      ...
      <p>Analyzing your claim against verified sources...</p>
    </div>
  `;
...
function showError(message) {
  ...
  resultsContent.innerHTML = `
    <div class="result-card verdict-false">
...
function showResult(result) {
  resultBadge.textContent = result.verdict === 'true' ? 'Verified' : result.verdict === 'false' ? 'Misleading' : 'Needs Review';
...
scanBtn.addEventListener('click', async () => {
  ...
  try {
    // DEMO MODE: Use local analysis engine
...
// Initialize page
console.log('Jigyasa News initialized. Ready to verify claims.');
